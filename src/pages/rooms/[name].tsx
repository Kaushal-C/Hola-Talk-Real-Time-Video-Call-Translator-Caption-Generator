...
import {
  ...
  useLocalParticipant,
  useRoomContext, // Import useRoomContext
} from "@livekit/components-react";
import {
  LogLevel,
  RoomOptions,
  VideoPresets,
  RoomState, // Import RoomState
} from "livekit-client";
import type { NextPage } from "next";
import { useRouter } from "next/router";
import { useEffect, useMemo, useState } from "react";
  ...
  return (
    <main data-lk-theme="default">
      {roomName && !Array.isArray(roomName) && preJoinChoices ? (
        ...
        <main className="grid min-h-[100dvh] w-full place-items-center p-4">
          <div className="flex w-full max-w-sm flex-col gap-4">
            <div className="flex flex-col gap-2 text-center">
              <h1 className= "text-2xl font-bold text-text-primary">Hey, {session?.user.name}!</h1>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-center text-sm text-text-primary">
    ...
    <>
      {data && (
        <LiveKitRoom
          ...
          options={roomOptions}
          video={userChoices.videoEnabled}
          audio={userChoices.audioEnabled} // Use the choice from pre-join
          onDisconnected={onLeave}
        >
          {/* --- FIX STARTS HERE --- */}
          <SetInitialMicrophoneState audioEnabled={userChoices.audioEnabled} />
...
function SetInitialMicrophoneState({ audioEnabled }: { audioEnabled: boolean }) {
  ...
  useEffect(() => {
    // Check if the room is connected and the participant exists
    if (room.state === RoomState.Connected && localParticipant) {
      // Only set the state if it differs from the pre-join choice
      if (localParticipant.isMicrophoneEnabled !== audioEnabled) {
        localParticipant.setMicrophoneEnabled(audioEnabled);
      }
    }
  }, [room.state, localParticipant, audioEnabled]); // Depend on the room's state
